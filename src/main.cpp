...
static String pageHtml() {
  ...
  h += F("<section><div class=\"muted\">通道（由软路由页面控制）</div><div id=\"chs\"></div></section>");
  h += F("<section><div class=\"muted\">WiFi 配置</div><div id=\"wl\" class=\"muted\">扫描中…</div>");
  h += F("<label>SSID</label><input id=\"ssid\"><label>密码</label><input id=\"pass\" type=\"password\">");
  h += F("<div class=\"row\"><button onclick=\"scan()\">重新扫描</button><button onclick=\"saveWifi()\">保存并连接</button>");
  ...
  h += F("async function scan(){let box=document.getElementById('wl');box.textContent='扫描中…';let l=await api('/api/scan');box.innerHTML='';");
  h += F("if(!l.networks.length){box.textContent='未发现可用网络';return;}");
  ...
  h += F("refresh();scan();setInterval(refresh,5000);");
