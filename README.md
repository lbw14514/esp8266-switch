...
> **接线前请先以插件里实际选定的引脚为准。** 下表只是出厂默认值，引脚可以在软路由页面的两个下拉框里自由更改；如果改了却不按新引脚接线，开机信号会打到错误的脚上。请先在插件里定好引脚，再对应接线。
...
可选引脚池只有 GPIO4 / 5 / 12 / 13 / 14 / 16 六个。GPIO0 / 2 / 15 是 ESP8266 的启动配置脚（分别需要高/高/低电平才能启动），接了外部电路容易导致开不了机；GPIO1 / 3 是串口；GPIO9 / 10 在多数模块上未引出，因此它们都不出现在下拉框里。
...
- 「扫描设备」用 `avahi-browse` 发现 `_esp-switch._tcp`，并行做 HTTP 探测确认在线状态并取回通道名与引脚
- 每 10 秒轮询一次已知设备（不做完整发现，避免卡顿）
- 发现未配对的新设备时弹窗询问「xxx 想要与您配对，是否同意」：同意即写入配置并同步设备名，不同意则加入忽略列表，之后不再弹窗
- 「移除」已配对设备同样进入忽略列表，列表里不再出现；顶部「恢复已忽略设备」可以一次性恢复
- 每台设备可自定义名称与三路通道名（默认 A/B/C），每路用两个下拉框选定引脚 A 与引脚 B
- 下拉框**只列出尚未被占用的引脚**，已被其它通道选走的引脚不会出现；选为空表示该路“未使用”
- 设备名自动去重，重名时自动加 `-2`、`-3` 后缀
